...
export default function CheckoutPage() {
  ...
  const formattedOrderItems = items.map((cartItem: any) => {
    const dbProduct = dbProducts.find(
      (p) => String(p.id || p._id) === String(cartItem.id),
    );

    const productName =
      cartItem.name || dbProduct?.name || "Fashion Collection Item";
    const origPrice = dbProduct
      ? Number(dbProduct.price)
      : Number(cartItem.price || 0);
    const discountPercent = dbProduct
      ? Number(dbProduct.discount) || 0
      : Number(cartItem.discount) || 0;
    const sellingPrice =
      discountPercent > 0
        ? origPrice - (origPrice * discountPercent) / 100
        : Number(cartItem.price) || origPrice;

    subtotalAfterProductDiscount +=
      sellingPrice * (Number(cartItem.quantity) || 1);

    let productImage = "";
    if (
      cartItem.image &&
      cartItem.image !== "No Image" &&
      !cartItem.image.includes("via.placeholder")
    ) {
  ...
  // 🚀 শিপিং চার্জ হিসাব
  const isInsideChattogram =
    selectedDistrict.toLowerCase().includes("chattogram") ||
    selectedDistrict.toLowerCase().includes("chittagong") ||
    selectedDivision.toLowerCase().includes("chattogram");
  const shippingInside =
    safeSettings.shippingInside !== undefined
      ? Number(safeSettings.shippingInside)
      : 60;
  const shippingOutside =
    safeSettings.shippingOutside !== undefined
      ? Number(safeSettings.shippingOutside)
      : 150;
  const shipping =
    items.length > 0
      ? isInsideChattogram
  ...
  const taxRate =
    safeSettings.taxRate !== undefined ? Number(safeSettings.taxRate) : 0;
  ...
  if (appliedCoupon) {
    if (appliedCoupon.discountType === "percentage") {
      finalCouponDiscountAmount =
        (totalBeforeCoupon * appliedCoupon.discountValue) / 100;
    } else {
      finalCouponDiscountAmount = appliedCoupon.discountValue;
    }
    if (finalCouponDiscountAmount > totalBeforeCoupon)
      finalCouponDiscountAmount = totalBeforeCoupon;
  }

  const totalAmount = Math.max(
    0,
    totalBeforeCoupon - finalCouponDiscountAmount,
  );
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    const customerEmail =
      formData.email.trim() || `${formData.phone.trim()}@mofashion.com`;
    ...
    try {
      ...
      toast.error(
        orderErr.message || "Failed to save order to Cloud Database!",
    ...
    try {
      const savedProducts = JSON.parse(
        localStorage.getItem("mo_fashion_products") || "[]",
      );
      for (const p of savedProducts) {
        const orderedItem = items.find(
          (i: any) => String(i.id) === String(p._id || p.id),
        );
        if (orderedItem) {
    ...
    try {
      if (appliedCoupon) {
        ...
        const targetCoupon = allCoupons.find(
          (c: any) => c.code === appliedCoupon.code,
        );
        if (targetCoupon) {
          const newUsedCount = (Number(targetCoupon.used) || 0) + 1;
          const newStatus =
            targetCoupon.usageLimit && newUsedCount >= targetCoupon.usageLimit
              ? "Expired"
              : targetCoupon.status;
          await saveSupabaseCoupon({
            ...targetCoupon,
            used: newUsedCount,
            status: newStatus,
          }).catch(() => null);
  ...
  const availableThanas =
    bdLocations[selectedDivision]?.[selectedDistrict] || [];

  return (
    <main className="min-h-screen py-10 text-white bg-[#111111] transition-all duration-300">
      <Helmet>
        <title>Checkout | {safeSettings?.storeName || "MO FASHION"}</title>
      </Helmet>

      <div className="container mx-auto px-4 max-w-7xl">
        <Link
          to="/cart"
          className="inline-flex items-center text-gray-400 hover:text-[#D4AF37] transition-all duration-200 mb-8 hover:-translate-x-1"
        >
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="lg:w-2/3 space-y-8">
            <div className="bg-[#1A1A1A] border border-[#D4AF37]/20 rounded-2xl p-6 sm:p-8 shadow-xl hover:border-[#D4AF37]/40 transition-all duration-300 backdrop-blur-md">
              ...
              >
                ...
                <div>
                  <label className="block text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">
                    Street Address (House/Road/Block) *
                  </label>
                  <input
                    type="text"
                    name="address"
                ...
                <div className="bg-[#111111] p-4 rounded-xl border border-gray-800 space-y-4">
                  <span className="text-xs text-[#D4AF37] font-bold uppercase tracking-wider flex items-center">
                    <Navigation size={14} className="mr-1.5" /> Select Delivery
                    Location Hierarchy
                  </span>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-gray-400 text-xs font-bold mb-1.5 uppercase">
                        1. Division (বিভাগ) *
                      </label>
                      <select
                    ...
                    <div>
                      ...
                      >
                        {availableThanas.map((thanaName) => (
                          <option key={thanaName} value={thanaName}>
                            {thanaName}
                          </option>
                        ))}
                      </select>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value="Bangladesh"
                      readOnly
                      className="w-full bg-[#1A1A1A] border border-gray-700 text-gray-400 rounded-xl px-4 py-3 cursor-not-allowed focus:outline-none text-sm font-semibold"
                    />
                  </div>
                </div>
              </form>
            ...
            <div className="bg-[#1A1A1A] border border-[#D4AF37]/20 rounded-2xl p-6 sm:p-8 shadow-xl hover:border-[#D4AF37]/40 transition-all duration-300">
              <h2 className="text-xl font-bold text-[#D4AF37] mb-6 uppercase tracking-wide border-b border-[#D4AF37]/10 pb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {safeSettings?.enableBkash !== false && (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-5 rounded-xl border transition-all duration-300 active:scale-95 ${paymentMethod === "bKash" ? "border-[#D4AF37] bg-[#D4AF37]/10 text-[#D4AF37] shadow-lg shadow-[#D4AF37]/10 scale-[1.02]" : "border-gray-800 text-gray-400 hover:border-[#D4AF37]/50 hover:text-white bg-[#111111]"}`}
                  >
                ...
                {safeSettings?.enableCard !== false && (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-5 rounded-xl border transition-all duration-300 active:scale-95 ${paymentMethod === "Card" ? "border-[#D4AF37] bg-[#D4AF37]/10 text-[#D4AF37] shadow-lg shadow-[#D4AF37]/10 scale-[1.02]" : "border-gray-800 text-gray-400 hover:border-[#D4AF37]/50 hover:text-white bg-[#111111]"}`}
                  >
                ...
                {safeSettings?.enableCOD !== false && (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-5 rounded-xl border transition-all duration-300 active:scale-95 ${paymentMethod === "Cash on Delivery" ? "border-[#D4AF37] bg-[#D4AF37]/10 text-[#D4AF37] shadow-lg shadow-[#D4AF37]/10 scale-[1.02]" : "border-gray-800 text-gray-400 hover:border-[#D4AF37]/50 hover:text-white bg-[#111111]"}`}
                  >
          ...
          <div className="lg:w-1/3">
            <div className="bg-[#1A1A1A] border border-[#D4AF37]/20 rounded-2xl p-6 sm:p-8 shadow-2xl sticky top-24 transition-all duration-300">
              <h2 className="text-xl font-serif font-bold text-[#D4AF37] mb-6 uppercase tracking-wide border-b border-[#D4AF37]/10 pb-4">
                Order Summary
              </h2>
              ...
              <div className="space-y-4 mb-6 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                {items.map((item: any, index: number) => (
                  <div
                    key={index}
                    className="flex items-center space-x-3.5 text-sm border-b border-gray-800/80 pb-3 last:border-0 group"
                  >
                    <div className="w-12 h-12 bg-[#111111] rounded-xl overflow-hidden flex-shrink-0 border border-gray-700 shadow-sm group-hover:scale-105 transition-transform">
                      {item.image && item.image !== "No Image" ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-[8px] text-[#D4AF37] uppercase font-bold">
                          No Img
                        </div>
                      )}
                    </div>
                    <div className="flex-1">
                      <span className="text-gray-200 line-clamp-1 font-semibold group-hover:text-[#D4AF37] transition-colors">
                        {item.name}
                      </span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-400">
                    ...
                    <span className="text-xs">
                      ({isInsideChattogram ? "Inside CTG" : "Outside CTG"})
                    </span>
                  </span>
                  <span className="text-white font-medium">
                    {safeSettings?.currency || "৳"} {shipping.toFixed(2)}
                  </span>
                </div>
                {taxRate > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-medium">
                      {safeSettings?.currency || "৳"} {taxAmount.toFixed(2)}
                    </span>
                  </div>
                )}
                {appliedCoupon && (
                  <div className="flex justify-between text-green-400 items-center bg-green-500/10 p-2.5 rounded-xl border border-green-500/20 mt-2 text-xs font-bold">
                    <span className="flex items-center">
                      <Tag size={14} className="mr-1.5" /> Coupon (
                      {appliedCoupon.code})
                    </span>
                    <span>
                      -{safeSettings?.currency || "৳"}{" "}
                      {finalCouponDiscountAmount.toFixed(2)}
                    </span>
                  </div>
                )}
              </div>
