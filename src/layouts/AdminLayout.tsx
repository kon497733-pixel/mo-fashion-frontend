...
export default function AdminLayout() {
  ...
  const handleVerifyPasscode = (e: React.FormEvent) => {
    ...
    const currentPasscode =
      localStorage.getItem("mo_admin_passcode") || "15987531";
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    const registeredEmail = (
      localStorage.getItem("mo_admin_email") || "kon497733@gmail.com"
    )
      .trim()
      .toLowerCase();
    ...
    const toastId = toast.loading(
      `Sending 5-minute security OTP code to ${registeredEmail}...`,
    );
    ...
    try {
      ...
      if (data && data.success) {
        setForgotStep("otp");
        toast.success(
          `Verification code sent to ${registeredEmail}! Check your Gmail inbox.`,
      ...
      toast.error(
        "Cannot connect to email server. Make sure backend server is running!",
        { id: toastId },
      );
    } finally {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.success) {
        toast.success("Identity Verified! Set your new passcode now.", {
          id: toastId,
        });
        setForgotStep("new_password");
      } else {
        toast.error("Incorrect OTP code entered! Please check your Gmail.", {
          id: toastId,
        });
      }
    } catch (e) {
  ...
  const handleSaveNewPassword = (e: React.FormEvent) => {
    ...
    toast.success(
      "New Admin Passcode set successfully! Please unlock dashboard.",
    );
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 bg-[#030303]">
          ...
          <div className="absolute inset-0 grid grid-cols-6 sm:grid-cols-10 md:grid-cols-14 grid-rows-8 sm:grid-rows-10 gap-1.5 p-2 opacity-25 z-1">
            {[...Array(140)].map((_, i) => (
              <div
                key={i}
                className="bg-[#0b0b0c] border border-gray-900 rounded-md"
              ></div>
            ))}
          </div>
          ...
          <form onSubmit={handleVerifyPasscode} className="space-y-6">
            <div className="relative group">
              <div className="relative mt-2">
                <Lock
                  size={18}
                  className="absolute left-4 top-4 text-[#D4AF37] z-10"
                />
                <input
                  type="password"
                  required
                  ...
                  className={`w-full bg-[#0A0A0A] border ${
                    passcodeError
                      ...
                      : "border-[#D4AF37]/40 focus:border-[#D4AF37] shadow-[0_0_15px_rgba(212,175,55,0.1)]"
          ...
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-[#1A1A1A] border-2 border-[#D4AF37]/40 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative text-left z-10 overflow-hidden">
              ...
              <div className="flex items-center space-x-3 mb-6 border-b border-gray-800 pb-4">
                ...
                <div>
                  <h3 className="font-serif font-bold text-lg text-[#D4AF37] uppercase">
                    Gmail OTP Recovery
                  </h3>
              ...
              {forgotStep === "email_input" && (
                <form onSubmit={handleSendOtp} className="space-y-5">
                  <p className="text-sm text-gray-300 leading-relaxed">
                    Enter your registered Admin Gmail address to receive the
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Mail
                        size={18}
                        className="absolute left-3.5 top-3.5 text-[#D4AF37]"
                      />
                      <input
                        type="email"
                        required
              ...
              {forgotStep === "otp" && (
                <form onSubmit={handleVerifyOtp} className="space-y-5">
                  <div className="flex items-center space-x-2 text-xs text-yellow-500 bg-yellow-500/10 p-3 rounded-lg border border-yellow-500/20">
                    ...
                    <span>
                      Check your Gmail inbox. OTP code is valid for 5 minutes.
                    </span>
                  </div>
              ...
              {forgotStep === "new_password" && (
                <form onSubmit={handleSaveNewPassword} className="space-y-4">
                  <p className="text-sm text-gray-300">
                    Set your custom new Admin Passcode (will unlock this
                    device):
                  </p>
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  size={20}
                  className={`transition-transform duration-300 group-hover:scale-110 ${isActive ? "text-[#D4AF37]" : ""}`}
                />
            ...
            <div className="flex items-center space-x-3 pl-5 border-l border-gray-800">
              {siteSettings?.logoUrl && siteSettings.logoUrl.trim() !== "" ? (
                <img
                  src={siteSettings.logoUrl}
                  alt="Logo"
                  className="w-9 h-9 object-contain rounded-full border border-[#D4AF37]/50 p-0.5 bg-[#111111]"
                />
              ) : (
