...
const supabaseUrl =
  ...
  "https://lcoujwhfddeihulurrwq.supabase.co";
const supabaseAnonKey =
  import.meta.env.VITE_SUPABASE_ANON_KEY ||
  "sb_publishable_Aib7MOvBq4kMBsiM7BeHnQ_ElMM9Cjl";
...
export const updateSupabaseSettings = async (
...
export const saveSupabaseProduct = async (productData: Record<string, any>) => {
  const targetId = String(
    productData.id || productData._id || `PROD-${Date.now()}`,
  );
  ...
  try {
    ...
    const filtered = Array.isArray(existing)
      ? existing.filter((p: any) => String(p.id || p._id) !== targetId)
      : [];
    const localPayload = { ...productData, id: targetId, _id: targetId };
    localStorage.setItem(
      "mo_fashion_products",
      JSON.stringify([localPayload, ...filtered]),
    );
...
export const deleteSupabaseProduct = async (id: string) => {
  ...
  try {
    ...
    const filtered = existing.filter(
      (p: any) => String(p.id || p._id) !== targetId,
    );
...
export const saveSupabaseCategory = async (
  categoryData: Record<string, any>,
) => {
  const targetId = String(
    categoryData.id || categoryData._id || `CAT-${Date.now()}`,
  );
  ...
  try {
    const existing = JSON.parse(
      localStorage.getItem("mo_fashion_categories") || "[]",
    );
    const filtered = Array.isArray(existing)
      ? existing.filter((c: any) => String(c.id || c._id) !== targetId)
      : [];
    const localPayload = { ...categoryData, id: targetId, _id: targetId };
    localStorage.setItem(
      "mo_fashion_categories",
      JSON.stringify([localPayload, ...filtered]),
    );
    window.dispatchEvent(new Event("storage"));
...
export const deleteSupabaseCategory = async (id: string) => {
  ...
  try {
    ...
    const filtered = existing.filter(
      (c: any) => String(c.id || c._id) !== targetId,
    );
...
export const saveSupabaseCoupon = async (couponData: Record<string, any>) => {
  const targetId = String(
    couponData.id || couponData._id || `COUPON-${Date.now()}`,
  );
  ...
  try {
    const existing = JSON.parse(
      localStorage.getItem("mo_fashion_coupons") || "[]",
    );
    const filtered = Array.isArray(existing)
      ? existing.filter((c: any) => String(c.id || c._id) !== targetId)
      : [];
    localStorage.setItem(
      "mo_fashion_coupons",
      JSON.stringify([{ ...payload, _id: targetId }, ...filtered]),
    );
    window.dispatchEvent(new Event("storage"));
  ...
  try {
    ...
    if (!error && Array.isArray(data)) {
      const formatted = data.map((order: any) => {
        const parsedItems = safeJsonParse(
          order.orderItems || order.order_items || order.cartItems,
        );
        const parsedSummary = safeJsonParse(
          order.orderSummary || order.order_summary,
        );
        const parsedInfo = safeJsonParse(
          order.customerInfo || order.customer_info,
        );

        return {
          ...order,
          orderItems: Array.isArray(parsedItems)
            ? parsedItems
            : parsedItems
              ? [parsedItems]
              : [],
...
export const saveSupabaseOrder = async (orderData: Record<string, any>) => {
  const targetId = String(
    orderData.id || orderData.orderId || orderData._id || `ORD-${Date.now()}`,
  );
  ...
  let itemsArray: any[] = Array.isArray(parsedItems)
    ? parsedItems
  ...
  try {
    const existing = JSON.parse(
      localStorage.getItem("mo_fashion_orders") || "[]",
    );
    const filtered = Array.isArray(existing)
      ? existing.filter(
          (o: any) => String(o.id || o.orderId || o._id) !== targetId,
        )
      : [];
    const localRecord = {
      ...payload,
    ...
    localStorage.setItem(
      "mo_fashion_orders",
      JSON.stringify([localRecord, ...filtered]),
    );
    window.dispatchEvent(new Event("storage"));
  ...
  if (error) {
    ...
    throw new Error(
      `Cloud Database Error: ${error.message} (${error.code || "RLS/Schema Block"})`,
    );
  }
...
export const deleteSupabaseOrder = async (id: string) => {
  ...
  try {
    ...
    const filtered = existing.filter(
      (o: any) => String(o.id || o._id || o.orderId) !== targetId,
    );
...
export const saveSupabaseCustomer = async (
  customerData: Record<string, any>,
) => {
  const targetId = String(
    customerData.id || customerData._id || `CUST-${Date.now()}`,
  );
  ...
  try {
    const existing = JSON.parse(
      localStorage.getItem("mo_fashion_customers") || "[]",
    );
    const filtered = Array.isArray(existing)
      ? existing.filter((c: any) => String(c.id || c._id) !== targetId)
      : [];
    localStorage.setItem(
      "mo_fashion_customers",
      JSON.stringify([{ ...payload, _id: targetId }, ...filtered]),
    );
    window.dispatchEvent(new Event("storage"));
...
export const moveToRecycleBin = async (
  originalTable: "products" | "categories" | "orders" | "coupons",
  item: Record<string, any>,
) => {
  const itemId = String(item.id || item._id || Date.now());
  const trashId = `TRASH-${Date.now()}`;
  const deletedAtStr = new Date().toLocaleDateString("en-GB", {
    day: "numeric",
    month: "short",
    year: "numeric",
    hour: "2-digit",
  ...
  try {
    ...
    const binKey =
      originalTable === "categories"
        ? "mo_fashion_recycle_bin_categories"
        : "mo_fashion_recycle_bin_products";
    const existingBin = JSON.parse(localStorage.getItem(binKey) || "[]");
    const cleanBin = existingBin.filter(
      (i: any) => String(i.id || i._id) !== itemId,
    );
    localStorage.setItem(
      binKey,
      JSON.stringify([
        { ...item, trashId, deletedAt: deletedAtStr },
        ...cleanBin,
      ]),
    );
...
export const restoreFromRecycleBin = async (
  ...
  try {
    if (originalTable && originalData) {
      ...
      await supabase
        .from(originalTable)
        .upsert([{ ...cleanData, id: targetId }], { onConflict: "id" });
    }
    if (trashId) {
    ...
    const activeKey =
      originalTable === "categories"
        ...
        : "mo_fashion_products";
    const activeItems = JSON.parse(localStorage.getItem(activeKey) || "[]");
    const cleanActive = activeItems.filter(
      (i: any) => String(i.id || i._id) !== targetId,
    );
    localStorage.setItem(
      activeKey,
      JSON.stringify([originalData, ...cleanActive]),
    );
    ...
    localStorage.setItem(
      binKey,
      JSON.stringify(
        existingBin.filter((i: any) => String(i.id || i._id) !== targetId),
      ),
    );
  ...
  try {
    ...
    [
      ...
      const filtered = items.filter(
        (i: any) =>
          String(i.trashId || i.id || i._id) !== String(trashId) &&
    ...
    if (originalTable) {
      ...
      const filtered = items.filter(
        (i: any) => String(i.id || i._id) !== String(itemId),
      );
